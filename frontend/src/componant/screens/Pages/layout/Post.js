    ...
    <article className="mt-2" ref={articleRef}  style={{border:"0.5px solid #dbdbdb",borderRadius:"10px"}}>
        ...
        <a>
          <div style={{cursor:'pointer'}}
            ...
            <div>
              ...
              {/* <img src="/images/icons/world-icon.png" style={{width:'15px', height:'15px'}}/> */}
              </div>
            </div>
          </div>
        </a>
        <div onMouseEnter={() => { setDropDown(!dropDown) }} onMouseLeave={() => { setDropDown(!dropDown) }}>
          ...
          <div className="bg-white rounded " style={{ width: 'fit-content', height: "auto", boxShadow: '1px 1px 3px 1px grey', position: "absolute", borderRadius: '0px',zIndex:1 }}>
            <ul>
              ...
              {userData._id == item?.users?._id && <li onClick={()=>{handleDelete(item?._id) }} className="fontcontent1 px-3 py-2 text-dark hovering">Remove</li>}
      ...
      <div id="post-interactions" >
        <div id="interactions-btns" className="py-1">
          {/* <div className="d-flex flex-column align-items-center ml-3 mt-1" style={{rowGap:'1px'}}>
            <img 
            src={`${userData?.profile ? `${BASE_URL + userData?.profile}` : `/images/profile/img0${userData?.randomprofile}.png`}`}
             style={{width:'22px',height:'22px',borderRadius:'50%'}}
             onError={handleImageError}
            />
          <div style={{color: "rgba(0 0 0 / 70%)",fontWeight: "500",fontSize:"12px"}}
          className="mt-0">
            Me
            </div>
          </div> */}
